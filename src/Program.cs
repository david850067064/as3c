    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                        ...
                        switch (args[i])
                        {
                            ...
                            default:
                                if (File.Exists(args[i]))
                                {
                                    ...
                                    {
                                        WriteInfo();
                                        Console.WriteLine("[-] Error: Can not open file {0}", args[i]);
#if DEBUG
                                        Console.WriteLine("[-] {0}", io.Message);
#endif
                                        return;
                                    }
                                    catch (Exception e)
                                ...
                                else
                                {
                                    ...
                                    {
                                        WriteInfo();
                                        Console.WriteLine("[-] File {0} does not exist.", args[i]);
                                        return;
                                    }
                                    else
                                    {
                ...
                {
                    WriteInfo();
                    Console.WriteLine("[-] Error: Invalid arguments. Use -help for help ...");
#if DEBUG
                ...
                switch (_action)
                {
                    ...
                    case Action.Replace:
                        ...
                        {
                            Console.WriteLine("[-] No valid ASM input given.");
                            return;
                        }

                        if ("" == _pathSwf || !File.Exists(_pathSwf))
                        {
                            Console.WriteLine("[-] No valid SWF target given.");
                            return;
                        }

                        if ("" == _className)
                        {
                            Console.WriteLine("[-] No class given.");
                            return;
                        }
                        ...
                        if (!_isConstructor)
                        {
                            if ("" == _methodName)
                            {
                                Console.WriteLine("[-] Need method name or constructor to replace");
                                return;
                            }
                        }
                        ...
                        Abc46 abc = null;
                        As3c.Swf.Abc.MethodInfo method = null;
                                    ...
                                    if (classFormat == instanceName)
                                    {
                                        ...
                                        if (_isMethodStatic)
                                        {
                                            if (_isConstructor)
                                            {
                                                if (null != body)
                                                {
                                                    Console.WriteLine("[-] Can not explicitly determine method body.");
                                                    return;
                                                }

                                                method = (As3c.Swf.Abc.MethodInfo)currentAbc.Methods[(int)classInfo.CInit];
                                            ...
                                            {
                                                Console.WriteLine("[-] Sorry, static methods do not work yet ...");
                                                return;
                                        ...
                                        else
                                        {
                                            if (_isConstructor)
                                            {
                                                ...
                                                method = (As3c.Swf.Abc.MethodInfo)currentAbc.Methods[(int)instanceInfo.IInit];
                                            ...
                                            else
                                            {
                                                ...
                                                for (int l = 0, p = instanceInfo.Traits.Count; l < p; ++l)
                                                {
                                                    ...
                                                    method = (As3c.Swf.Abc.MethodInfo)currentAbc.Methods[(int)methodBody.Method];
                        ...
                        if (null == body)
                        {
                            Console.WriteLine("[-] Could not find {0}.",(instanceFound)?"method":"class");
                            return;
                        }
                        ...
                        CompilerAs3c compiler = new CompilerAs3c();
                        compiler.Compile(currentAbc, parser.Instructions, parser.Labels);
                        ...
                        {
                            maxStack = ByteCodeAnalyzer.CalcMaxStack(abc, compiler.Code);
                        }
                        ...
                        {
                            localCount = ByteCodeAnalyzer.CalcLocalCount(compiler.Code);
                        }
                        ...
                        {
                            maxScopeDepth = ByteCodeAnalyzer.CalcScopeDepth(compiler.Code);
            ...
            catch (InstructionException iex)
            {
                ...
                Console.WriteLine("{0}({1}): {2}", iex.ParserInfo.FilePath, iex.ParserInfo.LineNumber, iex.ParserInfo.Line);
        ...
        {
            As3c.Swf.Abc.MethodInfo methodInfo = (As3c.Swf.Abc.MethodInfo)abc.Methods[(int)methodIndex];
